  ...
  <div class="detail-page">
    <div class="detail-header">
      ...
      <div class="detail-header__inner">
        <p class="p1">
          <span style="font-size: 15px;">{{bookDetails.title}}</span>
          <span>&nbsp;&nbsp;&nbsp;&nbsp;{{bookDetails.updateStatus}}</span>
        </p>
        <p class="p2">
          <em @click="gotoSearchPage(bookDetails.author)">{{bookDetails.author}}</em>
          <span v-if="bookDetails.cat">&nbsp;|&nbsp;{{bookDetails.cat}}</span>
          <span>&nbsp;|&nbsp;{{wordCount}}字</span>
        </p>
        <p class="p3">{{updatedTime}}</p>
      </div>
      <div class="detail-header__cooperation">
        <button @click="gotoChapterPage(bookDetails.id)">开始阅读</button>
        <button @click="afterMore(bookDetails.id)" v-if="!isAddToBookCase">+追更新</button>
      </div>
    </div>
    ...
    <!--</li>-->
    <!--</ul>-->
    <div class="detail-introduce" v-if="bookDetails.shortIntro">
      <h5>简介</h5>
      <div class="container">
    ...
    <div class="detail-likes" v-if="booksLike.length > 0">
      ...
      <scroll-view scroll-x style="width: 100%">
        <div class="like-item" @click="gotoDetailsPage(book.id)" v-for="(book, index) in booksLikeConver" :key="index">
    ...
    export default {
        ...
        methods: {
            ...
            getBookDetails(bookId) {
                ...
                const p = fetch(`https://${server}/api/bookDetail?bookId=${bookId}`, null,false);
                p.then((data) => {
            ...
            async afterMore(bookId) {
                ...
                // const {chapters} = await store.dispatch('fetchChapterList', bookId);
                // 默认第一章节
                if (Array.isArray(this.chapters) && this.chapters.length > 0) {
                    const currentChapter = this.chapters[0];
        ...
        computed: {
            ...
            // 字数
            wordCount() {
                const count = this.bookDetails.wordTotal;
                if (count > 10000) {
            ...
            // 更新时间
            updatedTime() {
                return formatTime(this.bookDetails.updateDate);
            },
            ...
            longIntroConver() {
                try {
                    // 暂时想不到其他法子，先把\n|\r转成<p>标签
                    return (this.bookDetails.shortIntro || '').replace(/\n|\r/g, '<p>');
                } catch (e) {
                    return '';
                }
            },
            ...
            // 是否被加入到书架了
            isAddToBookCase() {
                return this.bookCase.findIndex(book => book.id === this.bookDetails.id) !== -1;
            }
        },
        onLoad() {
            ...
            if (bookId) {
                ...
                store.dispatch('fetchChapterList', bookId).then(
                    ({chapters})=>{
        ...
        onShareAppMessage() {
            return {
                ...
                path: `/pages/details/main?bookId=${this.bookDetails.id}`,
