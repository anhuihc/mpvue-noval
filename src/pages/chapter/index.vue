<template>
  <div :class="['chapter-page', {nightMode: isNightMode}]">
    <scroll-view :style="{height:(heightX+50)+'px'}" scroll-y>
      <rich-text class="chapter-content" @click="onPageClick"
      ...
      <div class="turnPage" v-show="chapterDetailsConver">
        ...
        <div class="button next" @click="gotoTargeChapter(currentPageIndex + 1)"
             v-if="currentPageIndex < chaptersSectionCount - 1">下一章
        </div>
      </div>
    </scroll-view>
    <!-- 底部菜单 -->
    <div :class="['chapter-footbar', { show: showFooterBar,nightMode: isNightMode }]">
      <ul>
      ...
      <!-- 设置面板 -->
      <div :class="['setting-panel', {nightMode: isNightMode,show: showFooterBar && showSettingPanel}]"
        ...
        <div class="font-size setting-panel-normal">
          ...
          <i-input-number style="margin-left:18px" :value="chapterFontSize" min="12" max="32" @change="changeFontSize" />
<!--          <slider min="12" max="30" :value="chapterFontSize" step="1" color="#f5f5f5" activeColor="#4393e2"-->
<!--                  block-size="16" @change="changeFontSize"/>-->
        </div>
      </div>
    </div>
    <i-drawer mode="left" :visible="showDirectory" @close="toggleCategoryList">
      <view class="chapter-picker">
        <header @click.stop>
          <h3 style="font-size: 16px;">目录(共{{chaptersSectionCount}}章)</h3>
          <picker :range="chapterSectionArray" @change="chapterSectionArrayChange"
                  :value="currentChapterSectionIndex">
          <span class="picker">{{chapterSectionArray[currentChapterSectionIndex]}}
            <i class="iconfont icon-dropDown"></i></span>
          </picker>
        </header>
        <scroll-view :style="{height:heightX+'px'}" scroll-y>
          <ul @click.stop>
            <li
              :key="item.id"
    ...
    export default {
        ...
        computed: {
            ...
            bookInBookCase() {
                ...
                return this.bookCase.find(book => book.id === bookId);
            },
            // 书源
            sourceList() {
            ...
            chapterDetailsConver() {
                try {
                    // 暂时想不到其他法子，先把\n|\r转成<p>标签
                    return (this.chapterDetails.cpContent || this.chapterDetails.body).replace(/&nbsp;/g, '');
                } catch (e) {
                    return '';
                }
            },
        ...
        methods: {
            ...
            toggleNightOrDay() {
                ...
                wx.setNavigationBarColor({
                    frontColor: this.isNightMode ? '#ffffff' : '#000000',
                    backgroundColor: this.isNightMode ? '#000' : '#FEFEFE'
                });
            },
            // 切换设置面板
            toggleSettingPanel() {
        ...
        onLoad() {
            this.heightX = wx.getSystemInfoSync().windowHeight - 50;
            // 更改titleBar标题
            wx.setNavigationBarColor({
                frontColor: this.isNightMode ? '#ffffff' : '#000000',
                backgroundColor:this.isNightMode ? '#000' : '#FEFEFE',
        ...
        onShow() {
            ...
            this.bookInBookCase = this.bookCase.find(book => book.id === bookId);
            ...
            if (bookId) {
                ...
                store.dispatch('fetchChapterList', bookId).then((result) => {
                    ...
                    // 章节ID
                    this.chapterListDataId = _chapterListData.id;
                    // 章节数量
        ...
        onShareAppMessage() {
            ...
            return {
                title: `${title} - ${chapterTitle}——瓢虫阅读`,
                path: `/pages/chapter/main?from=share&bookId=${id}&chapterIndex=${this.currentPageIndex}`,
...
<style lang="scss">
  ...
  .chapter-picker {
    ...
    li {
      padding: 15px;font-size: 16px;color: #000;border-bottom: 1px solid #e5e5e5;
    }
    li.active {
      color: $primaryColor;
      background-color: transparent;
