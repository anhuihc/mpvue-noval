<template>
  <div class="source-page">
    <div class="source-item" v-for="(source, index) in formatTimeSourceList" :key="index" @click="changeSource(source)">
      ...
      <span class="tip" v-if="(bookInBookCase && bookInBookCase.currentSource && bookInBookCase.currentSource.id === source.id) || (!bookInBookCase && index === 0)">当前选择</span>
    </div>
  </div>
</template>
...
export default {
  computed: {
    ...
    // 书籍对应的书架的内容
    bookInBookCase() {
      return this.bookCase.find(book => book.id === this.chapterListData.book);
    },
    // 书源
    sourceList() {
